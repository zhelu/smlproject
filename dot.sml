...
structure Dot :> DOT = struct
  local
    ...
    val footer = "}"
  in fun levelPathCounterToDotGraph levelPathCounts (file : string) =
       let fun dectypeListComp ((xs,_),(ys,_)) =
             let val c = Int.compare (length xs, length ys)
             in if c = EQUAL
                then List.collate
                       (fn (x,y) =>
                         String.compare
                           (AstType.decTypeToString x,
                             AstType.decTypeToString y))
                       (xs,ys)
                else c
             end
