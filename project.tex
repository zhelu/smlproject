...
What features are common to expert ML programmers?
...
code accessed and modifies data in ways that might be considered undesirable
for production code. It seems fair therefore to exclude code used for
testings and other similar quality assessments from our overall analysis.
...
must await more analysis of the data.
...
The two primary reasons that the examples it finds are not necessary
opportunities for fold are that explicit recursion
...
in FoxNet contain a single \texttt{val} declaration for files in a directory. It is unclear
...
\begin{enumerate}
\item There is no general consensus on tabs vs.\ spaces. Some experts do make an effort at consistency.
...
\item Use list library functions when possible, especially \texttt{map}.
...
selected not based on what experts recommend but rather on
