...
Based on the potential advantages of using a fold, beginners
...
In this project, we've tried to find meaningful measurements of aspects of expert ML code
and then distill from these recommendations some concept of what good code
looks like.

We found that good style is, not surprisingly, subjective yet
difficult to conform to at the same time -- even when we can't find consensus on good,
we can find out the bad. In the end, we evaluated how well expert coders
conformed to a style rule if they followed it at all. We find that the authors
of the expert ML projects we looked at generally have a consistent style
but occassionally stray from it.

The other aspect we examined was how selected syntactic features in ML were used
by the experts. Specifically, we looked at library functions, functional strictness,
modularity, and the use of helper functions. In general, the expert projects
demonstrated with their fingers that they very much believe in their stated principles.

We find expert ML coders to be similar to one another in the aspects we looked at,
and we suspect that looking at other features will lead to similar results. But that
will need to be confirmed with additional work. Still, that the experts' code
share common features suggests that what is shared is a common thread of
expertise. Thus, beginners in ML are highly encouraged to think about the features
highlighted in this project and use them to shape their work as they continue
their journey from novicedom to the world of experts.
